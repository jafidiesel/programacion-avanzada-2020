...
router
  ...
  .post("/:hash/join", async function (req, res, next) {
    ...
    try {
      let result = await campaignService.joinCampaign(
        req.params.hash,
        req.body
      );
  ...
  .get("/:hash/history-board", function (req, res, next) {
    res.status = 500;
    res.send(
      buildResponse(500, "Endpoint not implemented. It's optional yet.", {})
    );
  })
  .post("/:hash/cell/:cell", async function (req, res, next) {
    //  post cell in board-N campaign-M
    try {
      let result = await campaignService.placeCell(
        req.params.hash,
        req.params.cell,
        req.body
      );
